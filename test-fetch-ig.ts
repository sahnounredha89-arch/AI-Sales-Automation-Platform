...
async function run() {
  ...
  const url = `https://graph.facebook.com/v19.0/${pageId}/conversations?platform=instagram&limit=25&fields=id,senders,updated_time,snippet,messages.limit(5){id,message,created_time,from,attachments}&access_token=${encodeURIComponent(token)}`;
  ...
  if (data.data) {
    ...
    for (const conv of data.data) {
      ...
      if (!isFromPage) {
        pendingCount++;
        console.log(`- Pending IG Conv: ${conv.id} | Customer: ${lastMsg.from?.name} | Last msg: "${lastMsg.message}" | Time: ${lastMsg.created_time}`);
      }
    }
